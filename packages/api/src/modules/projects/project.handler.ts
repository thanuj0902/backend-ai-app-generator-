...
import { buildPaginatedResponse, buildSuccessResponse, getPagination, etagFromData, notModifiedIfMatch } from "@/server/config"
...
async function getOwnedProject(projectId: string, userId: string) {
  const project = await prisma.project.findUnique({
    where: { id: projectId }, include: { _count: { select: { workflows: true } } },
  })
...
export async function listProjectsHandler(request: NextRequest) {
  try {
    ...
    const [projects, total] = await Promise.all([prisma.project.findMany({ where, skip, take: limit, orderBy: { createdAt: "desc" } }), prisma.project.count({ where })])
    const etag = etagFromData(projects)
    if (notModifiedIfMatch(request, etag)) return new Response(null, { status: 304 })
...
export async function createProjectHandler(request: NextRequest) {
  try {
    ...
    const project = await prisma.project.create({ data: { name: body.name, description: body.description || null, slug: slugify(body.name), userId: user.id, status: "DRAFT" as ProjectStatus } })
